 ...
 * $RCSfile: storage.h,v $
 * $Revision: 1.133 $ $Date: 2016/06/20 10:08:13 $
 * $State: Exp $ $Locker:  $
 *
...
#define INDEXTYPE 16
#define NUMARRAYTYPE 17

#define NUMERIC(xx) ((xx==INTEGERTYPE) | (xx==REALTYPE))

typedef unsigned int oidtype;                      /* Physical object handle */
...
struct stringcell                                      /* Fix length strings */
{
  ...
  unsigned short int shortlen;/* Length in bytes including terminating null
                                 character                                   */
  union {char string[sizeof(size_t)];
         size_t longlen;} cont;                 /* The string is padded here */
};
...
typedef size_t(*hash_function)(oidtype);             /* hash on object */
...
#define nil (oidtype)(2*OBJECTSIZE)
                                                          /* Lisp symbol NIL */
...
EXTERN char *a_typename(int id);                /* Get type name of type tag */
...
struct aligned_objectcell /* Template for double word aligned storage object */
{
   ...
   size_t bytes;              /* Total size of object in bytes, incl. header */
...
EXTERN size_t compute_hash_key(oidtype key);
...
#define IntoComplex(oid, re, im, env)\
    ...
    } else if (a_datatype(oid) == COMPLEXTYPE) {\
...
EXTERN int a_register_error(const char *msg);
...
EXTERN oidtype a_read_from_buffer(void *buffer, size_t len);
...
#define Int64ToString(x, buff) (sprintf(buff,"%lld",x),buff)
